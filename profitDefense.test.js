...
result = evaluateProfitDefense({ ...base, createdAt: new Date(Date.now() - 200 * 60000).toISOString() }, { percent: 0.02, usdt: 0.01 });
assert.equal(result.action, "TIME_FLAT_EXIT", "Süresi dolan başa baş işlem sermayeyi kilitlememeli");

result = evaluateProfitDefense({ ...base, tp1Done: true, createdAt: new Date(Date.now() - 200 * 60000).toISOString() }, { percent: 0.02, usdt: 0.01 });
assert.equal(result.action, null, "TP1 sonrası mevcut trailing yönetimi çalışmalı");

assert.equal(dollarStage(base, { percent: 0.5, usdt: 0.25 }), null);
assert.equal(dollarStage(base, { percent: 0.6, usdt: 0.30 }), "TP1");
assert.equal(dollarStage({ ...base, tp1Done: true }, { percent: 1.2, usdt: 0.60 }), "TP2");
assert.equal(dollarStage({ ...base, tp1Done: true, tp2Done: true }, { percent: 2.0, usdt: 1.00 }), "FINAL");
